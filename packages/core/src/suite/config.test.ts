...
describe('readProjectConfig', () => {
  it('reads .healix/config.json and returns {} when absent or invalid', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ auth: { loginUrl: '/login', roles: { default: { emailEnv: 'APP_QA_EMAIL', passwordEnv: 'APP_QA_PASSWORD' } } } }),
      'utf-8',
    );
...
describe('resolveTierBEnv', () => {
  ...
  it('absolute config loginUrl passes through untouched; no baseUrl leaves relative URL unresolved', () => {
    ...
    expect(resolveTierBEnv(absCfg, 'http://localhost:1', {} as NodeJS.ProcessEnv).HEALIX_TIERB_LOGIN_URL).toBe(
      'https://sso.corp.test/login',
    );
