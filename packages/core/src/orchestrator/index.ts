...
async function runPipeline(
  ...
  // Final coverage-feedback-loop ratio vs. its target (set only when the loop
  // actually ran — see "8c. COVERAGE FEEDBACK LOOP" below), so a run that fell
  // short of its 80%/98% target is visible in the report instead of only a
  // warn-level log line.
  let finalCoverage: { ratio: number; target: number } | null = null;
  ...
  try {
    ...
    try {
      ...
      const summary = await finalizeReport(
        ...
        noteStoreOk,
        noteStoreFailure,
        { generationStats, coverage: finalCoverage },
      );
    ...
    if (suiteMode === 'reuse' || !repoIndex?.functionality || repoIndex.functionality.length === 0) {
      ...
      finalCoverage = { ratio: coverage.ratio, target: coverageTarget };
