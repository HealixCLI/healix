...
/**
 * Windows: name the real binary so runCli spawns it DIRECTLY. runCli shells
 * through cmd.exe for non-.exe commands, and cmd.exe mangles unescaped
 * parens/brackets — which our commit messages ("(+1 spec) [run_x]") and the
 * `:(exclude)` diff pathspec both contain. POSIX spawns argv directly either way.
 */
