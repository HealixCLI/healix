...
import { getStore, type TestCase, type TestStatus } from '@healix/core';
...
export function registerReport(program: Command): void {
  program
    ...
    .action(async (runId: string) => {
      ...
      const runStatusColor = run.status === 'passed' ? pc.green : run.status === 'failed' ? pc.red : pc.yellow;
      ...
      if (tests.length === 0) {
        ...
        console.log('');
        const summaryParts = Object.entries(counts).map(([k, v]) => statusColor(k as TestStatus)(`${v} ${k}`));
