 ...
 *
 * Runs up to 3 requests in parallel.
 */
export async function generate(ctx: TestModeContext, plan: TestPlan): Promise<GeneratedSpec[]> {
  const items = plan.items ?? [];
  const GEN_CONCURRENCY = 3;
  emit(ctx, `Generating ${items.length} spec(s) (up to ${GEN_CONCURRENCY} in parallel)`, { count: items.length });
  ...
  const tasks = items.map((item, i) => async () => {
    const index = i + 1;
    emit(ctx, `[${index}/${items.length}] Generating "${item.title}"`, { id: item.id, tier: item.tier });
    const res = await generateOne(ctx, item, usedPaths);
    completed++;
  ...
  for (const o of outcomes) {
    if (!o.spec) {
      ...
      ctx.emit?.('generate', `Skipped "${o.item.title}": ${o.reason ?? 'generation failed'}`, {
        id: o.item.id,
        ...(o.violations ? { violations: o.violations } : {}),
      });
      continue;
    }

    specs.push(o.spec);
    emit(ctx, `Wrote ${o.spec.path}`, { title: o.spec.title });
  }
