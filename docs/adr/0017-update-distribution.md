...
## Decision
- **Packager:** **electron-builder**, configured in `apps/desktop/package.json` (`build` block). `pnpm --filter @healix/desktop package` produces an unpacked `release/mac-arm64/Healix.app`; `dist` targets dmg/nsis/AppImage per-OS.
