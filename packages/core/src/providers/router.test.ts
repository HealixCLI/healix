 ...
 *     fixed preference order (claude → openai), and
 *   - an unhealthy / unauthenticated preferred provider is SKIPPED so the router
 *     falls back to the next candidate (or null when none qualify).
 */
...
const ready = (status: HealthResult['status'] = 'ready', authenticated = true) =>
...
describe('ProviderRouter.select', () => {
  ...
  it('skips a preferred provider that is detected but NOT authenticated', async () => {
    const router = new ProviderRouter([
      makeClaude(ready('not-authenticated', false)),
      makeOpenAI(ready()),
    ]);
